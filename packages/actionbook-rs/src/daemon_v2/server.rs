...
/// or an I/O error occurs, allowing a single connection to serve multiple
/// sequential requests without reconnecting.
async fn handle_connection(mut stream: UnixStream, router: &Router) -> std::io::Result<()> {
    let mut len_buf = [0u8; 4];

    loop {
        // Read length-prefix (4 bytes LE). EOF here means client disconnected.
...
mod tests {
    use super::*;
    use crate::daemon_v2::action::Action;
    use crate::daemon_v2::action_result::ActionResult;
    use crate::daemon_v2::backend::TargetInfo;
    use crate::daemon_v2::registry::{SessionHandle, SessionRegistry, SessionState};
    use crate::daemon_v2::session_actor::SessionActor;
    use crate::daemon_v2::types::{Mode, SessionId};
    use crate::daemon_v2::wire::{self, Request};
    ...
    impl BackendSession for MockBackend {
        ...
        async fn checkpoint(&self) -> crate::error::Result<Checkpoint> {
            Ok(Checkpoint { kind: crate::daemon_v2::backend::BackendKind::Local, pid: Some(1), ws_url: "ws://m".into(), cdp_port: None, user_data_dir: None, headers: None })
        }
