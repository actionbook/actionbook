...
### Attach to a Running Chrome (Auto-Connect)

Use `--auto-connect` to attach to a Chrome instance you've already started — useful when you need your existing login sessions, extensions, or a specific Chrome configuration.

<Warning>
  Auto-connect requires **Chrome 144 or newer**. For older Chrome versions, use [Extension mode](#extension-mode-setup) instead.
</Warning>
...
<Note>
  `--auto-connect` (env: `ACTIONBOOK_AUTO_CONNECT`) is mutually exclusive with `--cdp-endpoint`, `-p/--provider`, `--mode cloud`, and `--mode extension`. Error codes: `CHROME_AUTO_CONNECT_NOT_FOUND` (no running Chrome found), `CHROME_CDP_UNREACHABLE` (Chrome found but CDP unreachable — verify `--remote-debugging-port` was set).
</Note>
