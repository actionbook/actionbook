...
actionbook browser stop --session s1                  # Stop local Chrome and retain its named profile
actionbook browser restart --session s1               # Restart, preserving session_id
```

<Note>
  `browser close` and `browser stop` are **idempotent** when the session and its profile-scoped ownership record are gone. A successful stop reports `status: "stopped"`, the profile name, and `profile_preserved: true`; a later `browser start --profile <profile>` reuses the retained browser state. If no profile ownership remains, the profile fields are `null` rather than claiming preservation.

  Ownership metadata that does not match the live PID, process start identity, and profile fails visibly instead of killing an unverified process. Another teardown already in flight returns `SESSION_CLOSING` fatal.
</Note>
