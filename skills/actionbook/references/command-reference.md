...
`--auto-connect` (env: `ACTIONBOOK_AUTO_CONNECT`) attaches to a Chrome you've started with remote debugging enabled. **Requires Chrome 144+** — for older versions, use extension mode instead. **Prerequisite**: start Chrome with `--remote-debugging-port`:
